...
- **一次可打多个**：`observations=[{...}, {...}]`；每个点自动分配 id（`w1`/`w2`…，单调递增、跨批次不复用），也可用 `{"id": "login"}` 自定义
- **`max`（默认 1）**：该点最多记几次命中，**达到后自动移除该点** —— 断点打在热点行也不会被反复中断页面；`max: 0` 表示不限（仍受全局上限：单点 20 条 / 总量 200 条）
- **列表**：`ved_watch()` 列出所有观测点（id / 类型 / 目标 / 命中数 / 状态）
- **删除**：`ved_watch(remove="w1,w3")`，`remove="all"` 清空
- JS 观测点的 `line` 是 **1-based**（和 `ved_script_source` 显示的行号一致）
**先读后打（JS 观测点）**：给脚本打断点前必须先用 `ved_script_source(url=...)` 读过它 ——
没读过会被拒绝，并给出"去读一次再重试"的恢复动作；读完之后脚本内容变了（页面重载）也要重读，
否则等于拿旧认知在错误的行上设断点。**请求观测点不涉及代码，没有这个要求。**
## 工具（29 个）
